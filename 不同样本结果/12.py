# -*- coding: utf-8 -*-
"""
HDMTL-style FW-UAV binary fault diagnosis
二分类版：0=normal，1=fault；CNN spatial branch -> Feature-wise Transformer / Spatial Self-Attention
...
7. Temporal Attention、Metric Loss 保持原逻辑；Soft Label 改为二分类 0.9/0.1。
...
AC_ID = '20'
DATA_FILE = r"D:\HDMTL-main\data\12_07_2020_Faulty_Daredevil_onboard_log\20_07_12__06_58_20_SD.data"
DATA_TYPE = "flight"
SAMPLE_PERIOD = 0.1
...
SELECTED_TOTAL_SAMPLES = 720

# 需要比较的不同小样本总数。
# 二分类时，代码会按类别均衡抽样：每类 SELECTED_TOTAL_SAMPLES / 2 个。
# 注意：这里的数值需要能被 NUM_CLASSES 整除，并且每类训练候选池样本数要足够。
SELECTED_TOTAL_SAMPLES_LIST = [90, 120, 150, 180, 270, 360, 720, 1440]

TRAIN_RATIO_ON_SELECTED = 0.70

# ===== 重复实验次数 =====
# 每一个 SELECTED_TOTAL_SAMPLES 下，固定 HDMTL_SAMPLE_SEED，重复训练 10 次不同初始化。
TIMES = 10

# 固定 HDMTL_SAMPLE_SEED：控制小样本 train/val 抽样，不随 time_idx 变化。
HDMTL_SAMPLE_SEED = 10

# ===== 结果保存目录 =====
RESULT_SAVE_DIR = "./hdmtl_sample_size_results"
...
# ===================== 8. 二分类标签生成逻辑 =====================
def assign_fault_binary(df):
    """
    二分类标签：
        0 = 正常
        1 = 故障

    判定逻辑沿用原始多分类代码中的故障触发条件：
        add1 超过阈值，或 add2 超过阈值，或 m1/m2 小于 1.0，均视为故障。

    与 9 分类版本不同：
        不再把 m2 < 0.9, 0.8, ..., 0.4 细分为不同严重等级，
        所有异常状态统一合并为 fault=1。
    """
    df = df.copy()
...
def load_and_prepare_data():
    ...
    df_time = assign_fault_binary(df_time)
...
def split_like_hdmtl(X_flat, y_seq, time_idx, verbose=True):
    ...
    # 固定 HDMTL_SAMPLE_SEED：
    # 对同一个 SELECTED_TOTAL_SAMPLES，训练/验证小样本划分保持一致；
    # TIMES 次实验只改变模型初始化，用于评估模型训练稳定性。
...
def get_soft_label_distribution(label, num_classes):
    ...
    其中主类权重为 SOFT_MAIN_WEIGHT=0.9，次类权重为 SOFT_NEIGHBOR_WEIGHT=0.1。
    """
    if num_classes != 2:
        raise ValueError(f"二分类软标签要求 num_classes=2，当前 num_classes={num_classes}")

    label = int(label)
    if label not in [0, 1]:
        raise ValueError(f"二分类标签只能是 0 或 1，当前 label={label}")

    dist = np.full(num_classes, SOFT_NEIGHBOR_WEIGHT, dtype=np.float32)
...
def run_experiment(config_name, time_idx, X_flat, y_seq, verbose=True):
    ...
    # Macro-F1：对每个类别分别计算 F1 后取平均，适合类别不均衡场景。
    ...
    # AUROC：二分类时使用 fault 类（类别 1）的预测概率。
    # 如果某次测试集中只出现一个类别，roc_auc_score 会报错，这里记为 NaN。
    try:
        if NUM_CLASSES == 2:
...
def mean_std(values):
    ...
    参数:
        values: list 或 np.ndarray，例如 10 次 test_acc / Macro-F1 / AUROC。
...
def trimmed_mean_std(values):
    ...
    if len(values) <= 2:
        raise ValueError("去掉最大值和最小值至少需要 3 个有效结果。")
    ...
    return mean_value, std_value, trimmed_values, min_removed, max_removed
...
def main():
    ...
    print("统计方式：每个样本数下重复训练 10 次，去掉最大测试值和最小测试值后计算平均值")
    print("=" * 80)
    ...
    # 推荐先跑 FT：Feature-wise Transformer + 1-layer LSTM + Temporal Attention + DSGF fusion
    ...
    for config_name in experiment_configs:
        ...
        for sample_num in SELECTED_TOTAL_SAMPLES_LIST:
            ...
            test_aurocs = [r["test_auroc"] for r in all_results]

            test_mean, test_std = mean_std(test_accs)
            val_mean, val_std = mean_std(val_accs)
            f1_mean, f1_std = mean_std(test_macro_f1s)
            auroc_mean, auroc_std = mean_std(test_aurocs)

            test_mean_trimmed, test_std_trimmed, test_accs_trimmed, test_min_removed, test_max_removed = trimmed_mean_std(test_accs)
            val_mean_trimmed, val_std_trimmed, val_accs_trimmed, val_min_removed, val_max_removed = trimmed_mean_std(val_accs)
            f1_mean_trimmed, f1_std_trimmed, f1_trimmed, f1_min_removed, f1_max_removed = trimmed_mean_std(test_macro_f1s)
            auroc_mean_trimmed, auroc_std_trimmed, auroc_trimmed, auroc_min_removed, auroc_max_removed = trimmed_mean_std(test_aurocs)
            ...
            # 保存 10 次测试值；如果以后 TIMES 改成其他数，也会自动生成对应列。
            ...
            for i, auroc in enumerate(test_aurocs, start=1):
                summary_row[f"test_auroc_{i}"] = auroc
            ...
            summary_row.update({
                ...
                # AUROC：去掉最大值和最小值后统计
                "test_auroc_min_removed": auroc_min_removed,
                "test_auroc_max_removed": auroc_max_removed,
                "test_auroc_trimmed_mean": auroc_mean_trimmed,
                "test_auroc_trimmed_std": auroc_std_trimmed,

                # 论文式写法
                "paper_accuracy": f"{test_mean_trimmed * 100:.2f}% ± {test_std_trimmed * 100:.2f}%",
                "paper_macro_f1": f"{f1_mean_trimmed:.4f} ± {f1_std_trimmed:.4f}",
                "paper_auroc": f"{auroc_mean_trimmed:.4f} ± {auroc_std_trimmed:.4f}",

                # 记录所有原始值和去极值后的值，便于复查
                "all_test_accs": str([round(x, 4) for x in test_accs]),
                "trimmed_test_accs": str([round(x, 4) for x in test_accs_trimmed.tolist()]),
                "all_val_accs": str([round(x, 4) for x in val_accs]),
                "trimmed_val_accs": str([round(x, 4) for x in val_accs_trimmed.tolist()]),
                "all_test_macro_f1s": str([round(x, 4) for x in test_macro_f1s]),
                "trimmed_test_macro_f1s": str([round(x, 4) for x in f1_trimmed.tolist()]),
                "all_test_aurocs": str([round(x, 4) for x in test_aurocs]),
                "trimmed_test_aurocs": str([round(x, 4) for x in auroc_trimmed.tolist()]),
            })

            summary_records.append(summary_row)

            print("\n" + "=" * 80)
            print(f"{config_name} | SELECTED_TOTAL_SAMPLES={sample_num} | {TIMES} 次重复实验结果")
            print("=" * 80)
            print("Test Accs:", [round(x, 4) for x in test_accs])
            print(f"10次 Test Acc 平均值: {test_mean:.4f} ± {test_std:.4f}")
            print("去掉最小值:", round(test_min_removed, 4))
            print("去掉最大值:", round(test_max_removed, 4))
            print("剩余 Test Accs:", [round(x, 4) for x in test_accs_trimmed.tolist()])
            print(f"去极值后平均测试准确率: {test_mean_trimmed:.4f} ± {test_std_trimmed:.4f}")
            print(f"论文式写法: Accuracy = {test_mean_trimmed * 100:.2f}% ± {test_std_trimmed * 100:.2f}%")

            print("\nMacro-F1s:", [round(x, 4) for x in test_macro_f1s])
            print(f"10次 Macro-F1 平均值: {f1_mean:.4f} ± {f1_std:.4f}")
            print("Macro-F1 去掉最小值:", round(f1_min_removed, 4))
            print("Macro-F1 去掉最大值:", round(f1_max_removed, 4))
            print("剩余 Macro-F1s:", [round(x, 4) for x in f1_trimmed.tolist()])
            print(f"去极值后 Macro-F1: {f1_mean_trimmed:.4f} ± {f1_std_trimmed:.4f}")

            print("\nAUROCs:", [round(x, 4) for x in test_aurocs])
            print(f"10次 AUROC 平均值: {auroc_mean:.4f} ± {auroc_std:.4f}")
            print("AUROC 去掉最小值:", round(auroc_min_removed, 4))
            print("AUROC 去掉最大值:", round(auroc_max_removed, 4))
            print("剩余 AUROCs:", [round(x, 4) for x in auroc_trimmed.tolist()])
            print(f"去极值后 AUROC: {auroc_mean_trimmed:.4f} ± {auroc_std_trimmed:.4f}")

    detail_df = pd.DataFrame(detail_records)
    summary_df = pd.DataFrame(summary_records)

    detail_csv = os.path.join(RESULT_SAVE_DIR, "detail_results_by_sample_size.csv")
    summary_csv = os.path.join(RESULT_SAVE_DIR, "summary_results_by_sample_size.csv")
    excel_file = os.path.join(RESULT_SAVE_DIR, "hdmtl_results_by_sample_size.xlsx")
    ...
    print(summary_df[[
        ...
        "test_auroc_mean",
        "test_auroc_std",
        "test_auroc_trimmed_mean",
        "test_auroc_trimmed_std",
        "paper_accuracy",
        "paper_macro_f1",
        "paper_auroc"
    ]])
